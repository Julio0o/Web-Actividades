...
// ─── SERVICES (cargados desde Django) ────────────────────────────────────────
...
function buildServices() {
  ...
  fetch('/api/servicios/')
    ...
    .then(function (data) {
      ...
      servicios.forEach(function (svc, idx) {
        var card = el('div', { classes: ['service-card', 'reveal'] });
        card.style.setProperty('--accent-color', svc.color);

        // Índice
        var index = el('span', { classes: ['service-index'], text: String(idx + 1).padStart(2, '0') });
      ...
      // Re-inicializar reveal para las nuevas tarjetas
      setTimeout(initReveal, 50);
    })
    .catch(function () {
      var err = el('p', {
        text: 'Error al cargar los servicios.',
        attrs: { style: 'color:#f43f5e;font-family:var(--font-mono);font-size:.82rem;' }
      });
